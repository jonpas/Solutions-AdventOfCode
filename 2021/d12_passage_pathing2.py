...
def walk(graph, paths, path, cons, small_twice=False):
    for con in cons:
        if con == "end":
            paths.append(path + ["end"])  # need a copy anyways
        elif con != "start":
            if con.isupper() or con not in path or not small_twice:
                path.append(con)
                walk(graph, paths, path, graph[con], small_twice=small_twice or (con.islower() and path.count(con) == 2))
                path.pop()
